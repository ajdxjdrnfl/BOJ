...
int checkMid(POINT a, POINT b, POINT c, POINT d){
     if(small(b, a)==1) swap(&a,&b);
    if(small(d,c)==1) swap(&c,&d);

    if((small(c,b)==1 && small(a,c)==1) || (small(a,d)==1 && small(c,a)==1) ||(small(d,b)==1 && small(a,d)==1) || (small(b,d)==1 && small(c,b)==1) ) return 1;
    else return 0;
}
void findPoint(POINT a, POINT b, POINT c, POINT d){
    long long x1 = a.x, x2 =b.x ,x3=c.x, x4=d.x, y1=a.y, y2=b.y, y3=c.y, y4= d.y;

    if(checkMid(a, b, c, d)==1) return;
    if(same(a,c)==1)  {
        printf("%lld %lld\n", a.x, a.y);
        return ;
    } 
...
int main(void){
    ...
    if(a==0 && b==0 && fourLine(p[0],p[1],p[2],p[3]) == 1) printf("0\n");
        ...
        printf("1\n");
        if( a==0 && b==0 && (same(p[0], p[2]) || same(p[0], p[3]) || same(p[1], p[2]) || same(p[1], p[3]))){
