...
int query(int *tree, int*prop,  int node, int start, int end, int obj){
    
    propaganda(tree, prop, node, start, end );
    if(start==end && start==obj) {
        return tree[node];
    }
    if(obj<start || end < obj) return 0;
    int mid = (start+end)/2;
    return query(tree, prop, node*2, start, mid, obj)+query(tree, prop, node*2+1, mid+1, end, obj);
}
...
int main(void){
    ...
    for(int i=0; i<m; i++){
        ...
        if(num==1){
            int a,b;
            scanf("%d %d", &a, &b);
            update(tree, prop,  1, 1, n, dfs_in[a], dfs_out[a], b);
        }
        else if(num==2){
            int a;
            scanf("%d", &a);
            printf("%d\n", query(tree, prop, 1, 1, n, dfs_in[a]));
        }
    }
}