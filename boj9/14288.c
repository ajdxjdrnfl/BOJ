...
int main(void){
    ...
    memset(downprop, 0 , sizeof(downprop));
     memset(uptree, 0 , sizeof(uptree));
    memset(upprop, 0 , sizeof(upprop));
    eulerTour(1);
    ...
    for(int i=0; i<m; i++){
        ...
        if(num==1){
            int a,b;
            scanf("%d %d", &a, &b);
            if(direct%2) update(uptree, upprop,  1, 1, n, dfs_in[a], dfs_out[a], b);
            else update(downtree, downprop, 1, 1, n, dfs_in[a], dfs_in[a],b);

        }
        else if(num==2){
            int a;
            scanf("%d", &a);
            printf("%d\n", query(uptree, upprop, 1, 1, n, dfs_in[a], dfs_in[a]) + query(downtree, downprop, 1, 1, n, dfs_in[a], dfs_out[a]));
        }
        else if(num==3){
            direct +=1;
        }
